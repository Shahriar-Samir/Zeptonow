...
const Product = (props:{params:{productName:string}}) => {
    ...
    useEffect(()=>{
        getProduct(productName.split('%20').join(' ').replace('%26','&'),setProduct)
    },[])    
    ...
    return (
        <main className='mt-5'>
            <section className='flex gap-14  px-5 '>
                ...
                <Image width={1000} height={1000} alt={product.name} className='w-3/4' src={product.img1}/>
                </figure>
                ...
                <article className='w-1/2'>
                    ...
                    <h1 className='font-font4 text-xl mt-3'>{product.name}</h1>
                    <Link href='#' className='text-[#950EDB] font-font3 text-md'>See all {product.company} products</Link>
                    <div className='flex flex-col mt-5 gap-5'>
                    <h1 className='text-gray-600'>{product.unit}</h1>
                    <div className='flex gap-3 items-center'>
                    <h1 className='font-font4 text-2xl'>₹{product.discountedPrice}</h1>
                    <h1 className='text-gray-600 line-through'>₹{product.price}</h1>
