import { subCategoryType } from "@/types";

export const teaCoffeeMore:subCategoryType[] = [
  {
      path: '/cn/tea-coffee-more/top-picks',
      img: '/category/SnacksAndDrinks/teaCoffeeMore/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/tea-coffee-more/coffee',
      img: '/category/SnacksAndDrinks/teaCoffeeMore/2.webp',
      title:'Coffee'
  },
  {
      path: '/cn/tea-coffee-more/tea',
      img: '/category/SnacksAndDrinks/teaCoffeeMore/3.webp',
      title:'Tea'
  },
  {
      path: '/cn/tea-coffee-more/drink-mixes',
      img: '/category/SnacksAndDrinks/teaCoffeeMore/4.webp',
      title:'Drink Mixes'
  },
  {
      path: '/cn/tea-coffee-more/green-herbal-tea',
      img: '/category/SnacksAndDrinks/teaCoffeeMore/5.webp',
      title:'Green & Herbal Tea'
  },
  {
      path: '/cn/tea-coffee-more/milk-drink-mixes',
      img: '/category/SnacksAndDrinks/teaCoffeeMore/6.webp',
      title:'Milk Drink'
  },
  {
      path: '/cn/tea-coffee-more/ready-to-drink',
      img: '/category/SnacksAndDrinks/teaCoffeeMore/7.webp',
      title:'Ready To Drink'
  },
]

// sweet-cravings
export const sweetCravings:subCategoryType[] = [
  {
      path: '/cn/sweet-cravings/top-picks',
      img: '/category/SnacksAndDrinks/sweetCravings/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/sweet-cravings/chocolates',
      img: '/category/SnacksAndDrinks/sweetCravings/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/sweet-cravings/indian-mithai',
      img: '/category/SnacksAndDrinks/sweetCravings/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/sweet-cravings/pastries-cakes',
      img: '/category/SnacksAndDrinks/sweetCravings/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/sweet-cravings/dessert-mixes',
      img: '/category/SnacksAndDrinks/sweetCravings/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/sweet-cravings/candies-gums-mints',
      img: '/category/SnacksAndDrinks/sweetCravings/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/sweet-cravings/premium-chocolates',
      img: '/category/SnacksAndDrinks/sweetCravings/7.webp',
      title:'Premium Chocolates'
  },
]


// munchies
export const munchies:subCategoryType[] = [
  {
      path: '/cn/sweet-cravings/top-picks',
      img: '/category/SnacksAndDrinks/munchies/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/sweet-cravings/chocolates',
      img: '/category/SnacksAndDrinks/munchies/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/sweet-cravings/indian-mithai',
      img: '/category/SnacksAndDrinks/munchies/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/sweet-cravings/pastries-cakes',
      img: '/category/SnacksAndDrinks/munchies/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/sweet-cravings/dessert-mixes',
      img: '/category/SnacksAndDrinks/munchies/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/sweet-cravings/candies-gums-mints',
      img: '/category/SnacksAndDrinks/munchies/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/sweet-cravings/premium-chocolates',
      img: '/category/SnacksAndDrinks/munchies/7.webp',
      title:'Premium Chocolates'
  },
]

// frozenFoods
export const frozenFoods:subCategoryType[] = [
  {
      path: '/cn/frozen-food/top-picks',
      img: '/category/SnacksAndDrinks/frozenFoods/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/frozen-food/chocolates',
      img: '/category/SnacksAndDrinks/frozenFoods/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/frozen-food/indian-mithai',
      img: '/category/SnacksAndDrinks/frozenFoods/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/frozen-food/pastries-cakes',
      img: '/category/SnacksAndDrinks/frozenFoods/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/frozen-food/dessert-mixes',
      img: '/category/SnacksAndDrinks/frozenFoods/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/frozen-food/candies-gums-mints',
      img: '/category/SnacksAndDrinks/frozenFoods/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/frozen-food/premium-chocolates',
      img: '/category/SnacksAndDrinks/frozenFoods/7.webp',
      title:'Premium Chocolates'
  },
]
// icecreams
export const icecreams:subCategoryType[] = [
  {
      path: '/cn/icecreams-more/top-picks',
      img: '/category/SnacksAndDrinks/icecreamsAndMore/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/icecreams-more/chocolates',
      img: '/category/SnacksAndDrinks/icecreamsAndMore/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/icecreams-more/indian-mithai',
      img: '/category/SnacksAndDrinks/icecreamsAndMore/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/icecreams-more/pastries-cakes',
      img: '/category/SnacksAndDrinks/icecreamsAndMore/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/icecreams-more/dessert-mixes',
      img: '/category/SnacksAndDrinks/icecreamsAndMore/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/icecreams-more/candies-gums-mints',
      img: '/category/SnacksAndDrinks/icecreamsAndMore/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/icecreams-more/premium-chocolates',
      img: '/category/SnacksAndDrinks/icecreamsAndMore/7.webp',
      title:'Premium Chocolates'
  },
]

// coldDrinks
export const coldDrinks:subCategoryType[] = [
  {
      path: '/cn/cold-drinks-juices/top-picks',
      img: '/category/SnacksAndDrinks/coldDrinks/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/cold-drinks-juices/chocolates',
      img: '/category/SnacksAndDrinks/coldDrinks/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/cold-drinks-juices/indian-mithai',
      img: '/category/SnacksAndDrinks/coldDrinks/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/cold-drinks-juices/pastries-cakes',
      img: '/category/SnacksAndDrinks/coldDrinks/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/cold-drinks-juices/dessert-mixes',
      img: '/category/SnacksAndDrinks/coldDrinks/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/cold-drinks-juices/candies-gums-mints',
      img: '/category/SnacksAndDrinks/coldDrinks/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/cold-drinks-juices/premium-chocolates',
      img: '/category/SnacksAndDrinks/coldDrinks/7.webp',
      title:'Premium Chocolates'
  },
]

// biscuits-cookies
export const biscuits:subCategoryType[] = [
  {
      path: '/cn/biscuits-cookies/top-picks',
      img: '/category/SnacksAndDrinks/biscuits/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/biscuits-cookies/chocolates',
      img: '/category/SnacksAndDrinks/biscuits/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/biscuits-cookies/indian-mithai',
      img: '/category/SnacksAndDrinks/biscuits/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/biscuits-cookies/pastries-cakes',
      img: '/category/SnacksAndDrinks/biscuits/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/biscuits-cookies/dessert-mixes',
      img: '/category/SnacksAndDrinks/biscuits/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/biscuits-cookies/candies-gums-mints',
      img: '/category/SnacksAndDrinks/biscuits/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/biscuits-cookies/premium-chocolates',
      img: '/category/SnacksAndDrinks/biscuits/7.webp',
      title:'Premium Chocolates'
  },
]


// 
export const atta:subCategoryType[] = [
  {
      path: '/cn/atta-rice-oils-dals/top-picks',
      img: '/category/GroceryAndKitchen/Atta/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/atta-rice-oils-dals/oil',
      img: '/category/GroceryAndKitchen/Atta/2.webp',
      title:'Oil'
  },
  {
      path: '/cn/atta-rice-oils-dals/atta-other-flours',
      img: '/category/GroceryAndKitchen/Atta/3.webp',
      title:'Atta & Other Flours'
  },
  {
      path: '/cn/atta-rice-oils-dals/dals-pulses',
      img: '/category/GroceryAndKitchen/Atta/4.webp',
      title:'Dals & Pulses'
  },
  {
      path: '/cn/atta-rice-oils-dals/rice-more',
      img: '/category/GroceryAndKitchen/Atta/5.webp',
      title:'Rice & More'
  },
  {
      path: '/cn/atta-rice-oils-dals/ghee',
      img: '/category/GroceryAndKitchen/Atta/6.webp',
      title:'Ghee'
  },
]


export const packaged:subCategoryType[] = [
  {
      path: '/cn/packagedFood/top-picks',
      img: '/category/GroceryAndKitchen/packaged/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/packagedFood/chocolates',
      img: '/category/GroceryAndKitchen/packaged/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/packagedFood/indian-mithai',
      img: '/category/GroceryAndKitchen/packaged/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/packagedFood/pastries-cakes',
      img: '/category/GroceryAndKitchen/packaged/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/packagedFood/dessert-mixes',
      img: '/category/GroceryAndKitchen/packaged/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/packagedFood/candies-gums-mints',
      img: '/category/GroceryAndKitchen/packaged/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/packagedFood/premium-chocolates',
      img: '/category/GroceryAndKitchen/packaged/7.webp',
      title:'Premium Chocolates'
  },
  {
      path: '/cn/packagedFood/premium-chocolates',
      img: '/category/GroceryAndKitchen/packaged/8.webp',
      title:'Premium Chocolates'
  },
  {
      path: '/cn/packagedFood/premium-chocolates',
      img: '/category/GroceryAndKitchen/packaged/9.webp',
      title:'Premium Chocolates'
  },
]


export const masala:subCategoryType[] = [
  {
      path: '/cn/masala-dry-fruitsdFood/top-picks',
      img: '/category/GroceryAndKitchen/masala/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/masala-dry-fruitsFood/chocolates',
      img: '/category/GroceryAndKitchen/masala/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/masala-dry-fruitsd/indian-mithai',
      img: '/category/GroceryAndKitchen/masala/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/masala-dry-fruits/pastries-cakes',
      img: '/category/GroceryAndKitchen/masala/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/masala-dry-fruitsd/dessert-mixes',
      img: '/category/GroceryAndKitchen/masala/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/masala-dry-fruits/candies-gums-mints',
      img: '/category/GroceryAndKitchen/masala/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/masala-dry-fruits/premium-chocolates',
      img: '/category/GroceryAndKitchen/masala/7.webp',
      title:'Premium Chocolates'
  },
  {
      path: '/cn/masala-dry-fruits/premium-chocolates',
      img: '/category/GroceryAndKitchen/masala/8.webp',
      title:'Premium Chocolates'
  },
  {
      path: '/cn/masala-dry-fruits/premium-chocolates',
      img: '/category/GroceryAndKitchen/masala/9.webp',
      title:'Premium Chocolates'
  },
]


export const breakfast:subCategoryType[] = [
  {
      path: '/cn/breakfast-saucesdFood/top-picks',
      img: '/category/GroceryAndKitchen/breakfast/1.webp',
      title:'Top Picks'
  },
  {
      path: '/cn/breakfast-saucesFood/chocolates',
      img: '/category/GroceryAndKitchen/breakfast/2.webp',
      title:'Chocolates'
  },
  {
      path: '/cn/breakfast-saucesd/indian-mithai',
      img: '/category/GroceryAndKitchen/breakfast/3.webp',
      title:'Indian Mithai'
  },
  {
      path: '/cn/breakfast-sauces/pastries-cakes',
      img: '/category/GroceryAndKitchen/breakfast/4.webp',
      title:'Pastries & Cakes'
  },
  {
      path: '/cn/breakfast-saucesd/dessert-mixes',
      img: '/category/GroceryAndKitchen/breakfast/5.webp',
      title:'Dessert Mixes'
  },
  {
      path: '/cn/breakfast-sauces/candies-gums-mints',
      img: '/category/GroceryAndKitchen/breakfast/6.webp',
      title:'Candies, Gums & Mints'
  },
  {
      path: '/cn/breakfast-sauces/premium-chocolates',
      img: '/category/GroceryAndKitchen/breakfast/7.webp',
      title:'Premium Chocolates'
  },
  {
      path: '/cn/breakfast-sauces/premium-chocolates',
      img: '/category/GroceryAndKitchen/breakfast/8.webp',
      title:'Premium Chocolates'
  },
  {
      path: '/cn/breakfast-sauces/premium-chocolates',
      img: '/category/GroceryAndKitchen/breakfast/9.webp',
      title:'Premium Chocolates'
  },
]